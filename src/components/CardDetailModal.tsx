      ...
      <div className="border-t border-border mt-2 pt-2">
        {showAdd ? (
          ...
          <button
            ...
            className="flex items-center gap-1.5 w-full px-2 py-1.5 rounded text-sm text-muted-foreground hover:bg-accent"
          >
...
export function CardDetailModal({
  ...
  const listObj = useKanbanStore((s) =>
    s.boards.find((b) => b.id === boardId)?.lists.find((l) => l.id === listId),
  );
  ...
  const tryAddAttachment = () => {
    const url = attachmentUrl.trim();
    if (!url) return;
    addAttachment(boardId, listId, card.id, {
      id: crypto.randomUUID(),
      name: attachmentName.trim() || url,
      url,
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-4 sm:pt-8 px-2 sm:px-4">
      ...
      <div className="relative bg-card rounded-xl shadow-2xl w-full max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden z-10 flex flex-col">
        ...
        <div className="flex items-center justify-end gap-1 px-4 pt-3 absolute top-0 right-0 z-10">
          <button
            onClick={onClose}
            className="p-1.5 rounded-md bg-card/80 hover:bg-accent text-muted-foreground hover:text-foreground"
          >
            <X size={18} />
          </button>
        </div>
            ...
            <div className="flex-1 p-4 sm:p-6 sm:pr-4 min-w-0">
                    ...
                    <>
                      ...
                      <div className="absolute top-full left-0 mt-1 z-50 bg-popover rounded-lg shadow-lg border p-2 w-64">
                        <div className="flex items-center justify-between px-2 py-1 mb-1">
                          ...
                          <button
                            onClick={() => setShowAddMenu(false)}
                            className="p-1 rounded-full border border-border hover:bg-accent text-muted-foreground"
                          >
                            <X size={14} />
                          </button>
                        </div>
                        <button
                          ...
                          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md hover:bg-accent text-left"
                        >
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  ...
                  <button
                    onClick={() =>
                      updateCard(boardId, listId, card.id, { coverColor: null })
                    }
                    className="px-2 h-6 text-xs rounded border border-border text-muted-foreground hover:bg-accent"
                  >
                    Clear
                  </button>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-2 py-1.5 text-sm border border-border rounded bg-secondary text-foreground placeholder:text-muted-foreground"
                  />
                  <button
                    onClick={() =>
                      updateCard(boardId, listId, card.id, {
                        coverImageUrl: coverImageInput.trim() || null,
                      })
                    }
              ...
              <div className="mb-6">
                ...
                {showAttachmentForm && (
                  <div className="bg-secondary rounded-lg p-2 mb-2 space-y-2">
                    <input
                      ...
                      className="w-full px-2 py-1.5 text-sm border border-border rounded bg-card text-foreground placeholder:text-muted-foreground"
                    />
                    <input
                      value={attachmentUrl}
                ...
                <div className="space-y-1.5">
                  ...
                  {card.attachments.map((a) => (
                    <div
                      key={a.id}
                      className="flex items-center justify-between gap-2 bg-secondary rounded px-2 py-1.5"
                    >
                      <a
                        href={a.url}
                        target="_blank"
                      ...
                      <button
                        onClick={() =>
                          removeAttachment(boardId, listId, card.id, a.id)
                        }
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {card.labels.map((l) => (
                      <span
                        ...
                        onClick={() =>
                          removeLabel(boardId, listId, card.id, l.id)
                        }
              ...
              {card.dueDate && (
                <div className="mb-4">
                  ...
                  <span
                    className={`text-sm px-2 py-1 rounded ${new Date(card.dueDate) < new Date() ? "bg-destructive/20 text-destructive" : "bg-secondary text-foreground"}`}
                  >
              ...
              <div className="mb-6">
                ...
                {editingDesc ? (
                  ...
                  <div
                    ...
                    className="min-h-[60px] px-3 py-2 text-sm rounded-lg bg-secondary cursor-pointer hover:bg-accent text-foreground"
                  >
                ...
                return (
                  <div key={cl.id} className="mb-6">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() =>
                          removeChecklist(boardId, listId, card.id, cl.id)
                        }
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (
                          e.key === "Enter" &&
                          (newItemTexts[cl.id] ?? "").trim()
                        ) {
                          addChecklistItem(boardId, listId, card.id, cl.id, {
                            id: crypto.randomUUID(),
                            text: newItemTexts[cl.id].trim(),
                            completed: false,
                          });
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                <button
                  onClick={() => {
                    updateCard(boardId, listId, card.id, { archived: true });
                    onClose();
                  }}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md bg-secondary hover:bg-accent text-muted-foreground"
                >
                  Archive
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md bg-destructive/10 hover:bg-destructive/20 text-destructive"
                >
            ...
            <div className="w-full sm:w-80 border-t sm:border-t-0 sm:border-l border-border p-4 shrink-0">
              ...
              <div className="mb-4">
                ...
                {commentText.trim() && (
                  <button
                    onClick={() => {
                      addComment(boardId, listId, card.id, {
                        id: crypto.randomUUID(),
                        memberId: "m1",
                ...
                {card.comments.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="flex gap-2">
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center text-primary-foreground text-[10px] font-bold shrink-0"
                        style={{ background: m?.color }}
                      >
                        {m?.avatar}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm bg-secondary rounded px-3 py-2 mt-1 text-foreground">
                          {c.text}
                        </p>
                      </div>
                    </div>
          ...
          <>
            ...
            <div className="absolute top-32 left-6 z-50 bg-popover rounded-lg shadow-lg border p-3 w-72">
              ...
              <input
                ...
                className="w-full px-2 py-1.5 text-sm border border-border rounded bg-secondary text-foreground mb-2 placeholder:text-muted-foreground"
              />
                  ...
                  return (
                    <div key={lc.color} className="flex items-center gap-2">
                      <input
                        ...
                        onChange={() => {
                          if (hasLabel) {
                            const label = card.labels.find(
                              (l) => l.color === lc.color,
                            );
                            if (label)
                              removeLabel(boardId, listId, card.id, label.id);
                          } else {
        ...
        {showLabelEditor && (
          <>
            ...
            <div className="absolute top-32 left-6 z-50 bg-popover rounded-lg shadow-lg border p-3 w-72">
              ...
              {/* Preview */}
              <div
                className="h-10 rounded-md mb-3 flex items-center px-3"
                style={{ background: editingLabelColor }}
              >
          ...
          <>
            ...
            <div className="absolute top-32 left-6 z-50 bg-popover rounded-lg shadow-lg border p-3 w-64">
              ...
              <div className="flex gap-1">
                ...
                <button
                  onClick={() => {
                    updateCard(boardId, listId, card.id, { dueDate: null });
                    setDueDate("");
                    setShowDatePicker(false);
                  }}
                  className="px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground"
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && newChecklistTitle.trim()) {
                    addChecklist(boardId, listId, card.id, {
                      id: crypto.randomUUID(),
