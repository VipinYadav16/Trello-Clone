...
export function KanbanList({
  ...
  const filteredCards = list.cards.filter((c) => {
    ...
    if (
      ...
      !c.labels.some((l) => filterLabels.includes(l.color))
    )
      return false;
    if (
      filterMemberIds.length > 0 &&
      !c.memberIds.some((id) => filterMemberIds.includes(id))
    )
      return false;
    if (filterDueDate) {
      ...
      if (filterDueDate === "today" && d.toDateString() !== now.toDateString())
        return false;
  ...
  const handleArchiveAll = () => {
    ...
    list.cards.forEach((c) =>
      updateCard(boardId, list.id, c.id, { archived: true }),
    );
    setShowMenu(false);
  };
            ...
            >
              {editing ? (
                <input
                  ...
                  onBlur={() => {
                    updateListTitle(boardId, list.id, title);
                    setEditing(false);
                  }}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                        ...
                        <>
                          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
                            <button
                              ...
                              className="p-0.5 rounded-full hover:bg-accent text-muted-foreground"
                            >
                              <ChevronLeft size={14} />
                            </button>
                          ...
                          <div className="p-3">
                            <div className="grid grid-cols-5 gap-2">
                              {LIST_COLORS.map((lc) => (
                                <button
                                  key={lc.color}
                                  onClick={() => {
                                    updateListColor(boardId, list.id, lc.color);
                                  }}
                            ...
                            {list.color && (
                              <button
                                onClick={() => {
                                  updateListColor(boardId, list.id, undefined);
                                }}
                        ...
                        <>
                          ...
                          <div className="p-1.5">
                            <button
                              ...
                              className="flex items-center gap-2 w-full px-3 py-2 text-sm text-foreground hover:bg-accent rounded-md"
                            >
                              Add card
                            </button>
                            <button
                            ...
                            <button
                              onClick={() => handleSortCards("created")}
                              className="flex items-center gap-2 w-full px-3 py-2 text-sm text-foreground hover:bg-accent rounded-md"
                            >
                              <ArrowUpDown
                                size={14}
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 w-full px-3 py-2 text-sm text-foreground hover:bg-accent rounded-md"
                            >
                              <div className="w-3.5 h-3.5 rounded-sm bg-gradient-to-br from-primary to-accent" />{" "}
                              Change list color
                            </button>
